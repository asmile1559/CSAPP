...
修改bits.c文件,使其能够通过在btest中的所有测试案例

# 2. 文件包含

```txt
Makefile - 编译 btest(测试程序), fshow(浮点数显示), and ishow(整数显示)  
README - 说明书
bits.c - 需要修改的文件 
bits.h - 头文件 
btest.c - 测试主程序  
...
dlc* - Rule checking compiler binary (data lab compiler) 编译器,用于编译bits.c  
[driver.pl](http://driver.pl/)* - Driver program that uses btest and dlc to autograde bits.c  编译器驱动
[Driverhdrs.pm](http://driverhdrs.pm/) - Header file for optional "Beat the Prof" contest 编译器驱动
fshow.c - 用于显示浮点数
ishow.c - 用于显示整数
```
...
会自动编译MakeFile文件,得到ishow和fshow程序
...
int conditional(int x, int y, int z)
{
  int m = ~0; // m是-1
  x = !x;
  return ((x + m) & y) | ((!x + m) & z); // 在x非零时y有效,否则z有效
}
/*
